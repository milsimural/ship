...
public class ShipController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Input.GetKeyDown(KeyCode.W)) CurrentSpeed = Mathf.Clamp(++CurrentSpeed, 0, MaxSpeed) / 100f;

        if(Input.GetKeyDown(KeyCode.S)) CurrentSpeed = Mathf.Clamp(--CurrentSpeed, 0, MaxSpeed) / 100f;
        ...
        if(CurrentSpeed != 0f) MoveShip(CurrentSpeed);
    }
